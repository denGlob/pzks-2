...
object Scheduler4 : Scheduler() {
    override fun getTaskAndCPUForPlacement(currentTick: Int, readyToPlaceTasks: List<Task>, system: System): Pair<Task, CPU> {
        ...
        val mostFreeCPU = freeCPUs.minBy { cpu ->
            val requiringTasks = task.parentTasks.entries

            return@minBy requiringTasks.sumBy { (requiringTask, difficulty) ->
                val taskHolderCPU = system.getPlacedCPUForTask(requiringTask)
                val hopsAmount = system.getPath(taskHolderCPU, cpu).size - 1
                return@sumBy difficulty * hopsAmount
            }
        }
...
object Scheduler3 : Scheduler() {
    override fun getTaskAndCPUForPlacement(currentTick: Int, readyToPlaceTasks: List<Task>, system: System): Pair<Task, CPU> {
        val task = readyToPlaceTasks.first()
        val freeCPUs = system.cpus.filter { it.alu.timeline.getWorkAt(currentTick) == null }.toMutableList()
        val mostConnectivityCpu = freeCPUs.maxBy { it.ioUnit.externalLinks.size }!!
        return task to mostConnectivityCpu
    }
}
